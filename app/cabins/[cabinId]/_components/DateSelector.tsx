...
function DateSelector({ settings, bookedDates, cabin }: any) {
  ...
  return (
    <div className="flex flex-col justify-between">
      <DayPicker
        className="py-2 place-self-center"
        mode="range"
      ...
      <div className="flex items-center justify-between px-8 bg-accent text-primary h-[72px]">
        ...
        {range.from || range.to ? (
          <button
            className="border border-primary py-2 px-4 text-sm font-semibold"
            onClick={() => resetRange()}>
            Clear
          </button>
        ) : null}
